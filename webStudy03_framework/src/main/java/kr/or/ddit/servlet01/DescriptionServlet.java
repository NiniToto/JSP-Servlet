 ...
 * 			ex) /test/*.do (x) 두가지 매핑 방식을 섞어서 사용할 수 없다.
 * 				'/'없이 test/sample(x) 안된다 -> 반드시 절대 경로 표기방식을 사용함.
 * 	5) WAS 재구동
 * 
 * 2. W.A.S의 역할
 * 	:Server, WAS(Web Application(context) Server), 
 * 	 Container(Web Container, Servlet Container : 내부의 컴포넌트의 라이프사이클을 관리하는 관리자)
 * 	lifecycle 내에서 각 시점 별로 호출할 수 있는 callback 메서드가 서블릿 스펙에 정의되어 있음.
 * 	
 * 	lifecycle callback
 * 	초기화 : 아무런 설정(load-on-startup)이 없는 한, 해당 서블릿에 대한 최초의 요청이 발생하면 초기화 진행.
 * 	소멸 : 아무런 설정이 없는 한, 서버는 서블릿을 싱글턴의 형태로 관리함.
 * 	request callback
 * 	service : 모든 request method에 공통적으로 적용될 코드를 정의함.
 * 	doXXX : 특정 request method에 대해서만 적용할 코드를 정의함.
 * 	
 */
public class DescriptionServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		String iniName = config.getInitParameter("iniName");
		String filePath = getClass().getResource(iniName).getFile();
