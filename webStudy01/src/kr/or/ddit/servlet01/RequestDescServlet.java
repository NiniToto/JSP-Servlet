 ...
 * 		- GET (R) : 조회, 일반적으로 클라이언트의 기본 요청 메서드, Body 영역이 형성되지 않음.
 * 		- POST (C) : 생성, 서버측으로 전달한 컨텐츠를 갖고 있는 요청으로 Body 영역이 형성됨.
 * 		- PUT (U) : 수정, method = "post", _method="put" 파라미터를 추가함.
 * 		- DELETE (D) : 삭제, method = "post", _method="delete" 파라미터를 추가함.
 * 		- HEADER : 응답데이터를 헤더만 받겠다.
 * 		- OPTION : preflight 요청에 사용, 서버의 메서드 이원여부를 확인할 용도. 
 * 		- TRACE : 서버를 디버딩할 용도.
...
public class RequestDescServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//percent encoding, URL encoding방식으로 인코딩되도록 정의되어 있음.
